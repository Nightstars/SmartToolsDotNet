...
using Codeless.Core.Factory;
using Codeless.Core.Models;
using Codeless.Core.Utils;
using Codeless.Core.Services;

namespace CodelessUI.ViewModels
{
    ...
    public class CodeLessViewModel : BindableBase
    {
        ...
        private void OnGenCode(Object obj)
        {
            ...
            try
            {
                ...
                var tbname = $"{ FieldUtil.GetModelName(_dbTable) }";

                var temp = _buildpath;

                var instance = new CodeBuilderFactory().GetInstance(
                    DbTableInfos,
                    _rootnamespace,
